...
Size computeActualSize(
  ...
  double actualWidth =
      max(actualStep, actualStep * (viewSize.width / actualStep).floor()) *
          actualDpr;
  ...
  if (intrinsic != null &&
      (actualHeight > intrinsic.height! || actualWidth > intrinsic.width)) {
    double actualRatio = actualWidth / actualHeight;
  ...
  return Size(
      width: actualWidth.roundToDouble(), height: actualHeight.roundToDouble());
}

Alignment computeAlignment(
    {Alignment? anchor, Alignment? alignment, BoxFit? fit}) {
...
String computePreTransform({
  ...
  final finalTransform =
      ((fit == BoxFit.cover) && (refit != null)) ? null : '/*';
  return '${preTransform != null ? '/$preTransform' : ''}${actualFocus != null ? '/focus=$actualFocus' : ''}${actualRefit != null ? '/refit=$actualRefit' : ''}${finalTransform ?? ''}';
}

String? computeRefit({Alignment? anchor, required BoxFit fit, String? refit}) {
  if (refit == null) {
    return null;
  }
  return '${(fit == BoxFit.contain) ? 'auto' : 'WxH'}${refit.isEmpty ? '' : '($refit)'}${(anchor == null || fit == BoxFit.contain) ? '' : '@${mappingAlignment[anchor]}'}';
}
...
Size computeViewSize({
  ...
  if (ratio == 0 &&
      (height == double.infinity || (height != null && height == 0))) {
