...
String createUrl(
  ...
  String actualOutput =
      (output != null && output.isNotEmpty) ? '/output=$output' : '';
  ...
  String actualTransform =
      computeTransform(context: context, transform: transform);
  return '${domain}/${((parsed.group(SPECIAL) != null || parsed.group(RESERVED) != null)) ?
      // old syntax
      '${VERSION}${actualTransform}${actualOutput}/${isMedia ? '${parsed[MEDIA]}:${actualPath}' : actualPath}${actualQuality}' :
      // catch-all syntax
      '${actualPath}${parsed.group(QUERY) != null ? '&' : '?'}twic=${VERSION}${actualTransform}${actualOutput}${actualQuality}'}';
}
